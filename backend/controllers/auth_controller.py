...
def register():
  ...
  # Generate a unique token from email ID with 1-sec expiration
        token = jwt.encode(
            {"email": new_user.email, "exp": datetime.now(timezone.utc) + timedelta(seconds=330)},
        ...
        rtoken = jwt.encode(
        {"email": new_user.email, "exp": datetime.now(timezone.utc) + timedelta(minutes=20)},
        ...
        return jsonify({"message": "OTP sent successfully.", "token": token,"ref_token":rtoken}), 200
...
def refresh_token():
    try:
        ...
        access_token = jwt.encode({"userId": str(user.id), "exp": datetime.now(timezone.utc) + timedelta(days=1)}, os.getenv("AUTH_SECRET"), algorithm="HS256").decode('utf-8')
...
def verifyOtp():
    try:
        ...
        # Generate JWT tokens
        access_token = jwt.encode({"userId": str(user.id)}, os.getenv("AUTH_SECRET"), algorithm="HS256")
        refresh_token = jwt.encode({"userId": str(user.id)}, os.getenv("REFRESH_TOKEN_SECRET"), algorithm="HS256")
...
def forgot_password():
    try:
        ...
        if(user.password==hashed_new_password):
            return jsonify({"error": "You can't set new password same as your current password ! "}), 403
