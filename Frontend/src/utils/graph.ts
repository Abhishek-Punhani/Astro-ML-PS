...
Chart.register(CategoryScale, LinearScale, PointElement, LineElement, LineController, ScatterController, Tooltip, zoomPlugin);
...
function plotGraph2(
    ...
    const zoomGestures = {
        ...
        limits: {
            ...
            y: { min: Math.min(...Y, ...lefty) - 10, max: Math.max(...Y, ...lefty) + 10 },
        }
    };
...
function plotGraph3(
    ...
    const zoomGestures = {
        ...
        limits: {
            x: { min: 0, max: X.length - 1 },
            // Adjust y limits to include both Y and lefty values
            y: { min: Math.min(...Y, ...righty) - 10, max: Math.max(...Y, ...righty) + 10 },
        }
    };
    ...
    const chart3 = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                ...
                {
                    label: 'Rising time Flux',
                    data: rightx.map((xValue, index) => ({ x: xValue, y: righty[index] })),
                    backgroundColor: 'rgba(0, 255, 0, 1)',
                    borderColor: 'rgba(0, 128, 0, 1)',
                    pointRadius: 8, // Make points larger for visibility
                    pointStyle: 'circle',
